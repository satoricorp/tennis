...
tennis put agents < events.ndjson
tennis put agents --openai text-embedding-3-small < events.ndjson
tennis put agents --json < events.ndjson
```

Each line is `{"id": "...", "text": "...", "attributes": {...}}`. `attributes` is optional; everything else follows `add`, including creating the namespace on first use and skipping documents that are byte-identical to what is stored.
...
```bash
$ tennis put agents --json < events.ndjson
tennis: put: line 3: unexpected end of JSON input
{
  "chunks": 2,
  "failed": 1,
  "skipped": 0,
...
tennis get agents e1 --json                    # with its attributes
