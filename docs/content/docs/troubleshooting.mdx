...
## "N line(s) failed to parse" from put
...
Bad lines are reported with their line number and skipped; the good ones are still written. `put` exits nonzero so a script notices. Each line needs `id` and `text`.
...
```bash
tennis ns drop notes          # one namespace
