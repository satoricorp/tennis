 ...
 * comes out an ellipse. Two pixels to a cell and the pixels are square, the
 * ball is round, and the wordmark gets twice the vertical resolution it would
 * otherwise have — which is the difference between Garamond's italic and a
 * smudge.
 *
 * The pair is then written as one character. Three of them do the whole mark,
 * chosen because of where their ink sits in the cell rather than how much of it
 * there is: a quote rides at the top, an underscore lies along the bottom, and
 * a hash fills the middle. So a half-lit cell is drawn by a character that is
 * lit on the same half, and the resolution survives the translation.
 *
 ...
 *
 * The output is checked in, as lines ready to print, and it is plain ASCII:
 * there is nowhere it can arrive as mojibake. A CLI that rasterises a font and
 * raycasts a sphere to say hello is a CLI carrying a font rasteriser and a
 * sphere raycaster.
 */
...
// What a cell is drawn with, by which of its halves are inked.
const FULL = "#";
const TOP = '"';
const BOTTOM = "_";
...
for (let j = 0; j + 1 < art.length; j += 2) {
  ...
  for (let i = 0; i < cols; i++) {
    ...
    line += top && bottom ? FULL : top ? TOP : bottom ? BOTTOM : " ";
  }
